...
import com.ute.foodiedash.infrastructure.persistence.restaurant.jpa.mapper.RestaurantCategoryMapJpaMapper;
import com.ute.foodiedash.infrastructure.persistence.restaurant.jpa.repository.RestaurantCategoryMapJpaRepository;
...
public class RestaurantCategoryMapRepositoryAdapter implements RestaurantCategoryMapRepository {
    ...
    public RestaurantCategoryMap save(RestaurantCategoryMap categoryMap) {
        RestaurantCategoryMapJpaEntity jpaEntity;
        if (categoryMap.getId() == null) {
            jpaEntity = mapper.toJpaEntity(categoryMap);
        } else {
            jpaEntity = jpaRepository.findById(categoryMap.getId())
                    .orElseThrow();
            mapper.updateEntity(jpaEntity, categoryMap);
        }
