...
import com.ute.foodiedash.infrastructure.persistence.promotion.jpa.mapper.PromotionUsageJpaMapper;
import com.ute.foodiedash.infrastructure.persistence.promotion.jpa.repository.PromotionUsageJpaRepository;
...
public class PromotionUsageRepositoryAdapter implements PromotionUsageRepository {
    ...
    public PromotionUsage save(PromotionUsage usage) {
        PromotionUsageJpaEntity jpaEntity;
        if (usage.getId() == null) {
            jpaEntity = mapper.toJpaEntity(usage);
        } else {
            jpaEntity = jpaRepository.findById(usage.getId())
                    .orElseThrow();
