...
import com.ute.foodiedash.infrastructure.persistence.restaurant.jpa.mapper.RestaurantBusinessHourJpaMapper;
import com.ute.foodiedash.infrastructure.persistence.restaurant.jpa.repository.RestaurantBusinessHourJpaRepository;
...
public class RestaurantBusinessHourRepositoryAdapter implements RestaurantBusinessHourRepository {
    ...
    public RestaurantBusinessHour save(RestaurantBusinessHour businessHour) {
        RestaurantBusinessHourJpaEntity jpaEntity;
        if (businessHour.getId() == null) {
            jpaEntity = mapper.toJpaEntity(businessHour);
        } else {
            jpaEntity = jpaRepository.findById(businessHour.getId())
                    .orElseThrow();
